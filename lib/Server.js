...
class TusServer extends EventEmitter {
     ...
     */
    handle(request, h) {
        console.log(`[TusServer] handle: ${request.method} ${request.url}`);
        ...
        if (request.method.toUpperCase() === 'GET') {
            return this.handlers.GET.send(request, h);
        }
        ...
        for (const header_name in request.headers) {
            ...
            if (header_name.toLowerCase() === 'content-type' && request.method !== 'patch') {
                continue;
            }
        ...
        if (this.handlers[request.method.toUpperCase()]) {
            return this.handlers[request.method.toUpperCase()].send(request, h);
        }
