...
class PatchHandler extends BaseHandler {
    ...
    send(request, h) {
        ...
        if (file_id === false) {
            return super.send(h, ERRORS.FILE_NOT_FOUND.status_code, {}, ERRORS.FILE_NOT_FOUND.body);
        }

        // The request MUST include a Upload-Offset header
        let offset = request.headers['upload-offset'];
        if (offset === undefined) {
            return super.send(h, ERRORS.MISSING_OFFSET.status_code, {}, ERRORS.MISSING_OFFSET.body);
        }

        // The request MUST include a Content-Type header
        const content_type = request.headers['content-type'];
        if (content_type === undefined) {
            return super.send(h, ERRORS.INVALID_CONTENT_TYPE.status_code, {}, ERRORS.INVALID_CONTENT_TYPE.body);
        }
        ...
        return this.store.getOffset(file_id)
            ...
            .then((new_offset) => {
                ...
                return super.send(h, 204, headers);
            })
            .catch((error) => {
